 ...
 *  - SiblingUid 是其兄弟节点存储在 DM 中的 UID。(Sibling:兄弟姐妹)
 *  - 后续是穿插的子节点（SonN）和 KeyN。
 *  - 最后的一个 KeyN 始终为 MAX_VALUE，以此方便查找。son值是UID，key值是 where条件搜索的值
 *
 * 这个Node类主要就是那两个方法，searchNext() 和 searchRange()；
 * searchNext() ：提供给B+树的底层搜索方法，就是一颗多叉搜索树二分搜索算法，
 *    目标值小于当前节点值的走左孩子，大于等于的继续for，这个节点都没找到满足要求的就去下一个兄弟节点找
 * searchRange()：提供给B+树的底层'范围'搜索方法。
 */
public class Node {
    // 节点头部信息偏移量
    ...
    static final int BALANCE_NUMBER = 32;                   // 平衡数 32
    ...
    /** 拷贝Node的第kth个孩子节点为 to */
    static void copyRawFromKth(SubArray from, SubArray to, int kth) {
        int offset = from.start+NODE_HEADER_SIZE+kth*(8*2);
        System.arraycopy(from.raw, offset, to.raw, to.start+NODE_HEADER_SIZE, from.end-offset);
    }
    /** 从Node的第kth个还孩子开始，整体平移到最后，空出当前位置 */
    ...
    public SearchNextRes searchNext(long key) {
        ...
        try {
            ...
            for(int i = 0; i < noKeys; i ++) {
                ...
                if(key < ik) {      // 根据排序搜索树的规则，小于的就往左下走就行了
